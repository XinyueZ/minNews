...
public final class WrapImageTextView extends RelativeLayout {

	public WrapImageTextView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}


	public WrapImageTextView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}


	public WrapImageTextView(Context context) {
		super(context);
		init(context);
	}
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (distance < 10) {
					if (event_code == MotionEvent.ACTION_UP) {
						WrapImageTextView.this.onClick(event.getX(), event.getY());
					}
					return true;
				} else {
					return false;
	...
	private final ArrayList<Box> mLineboxes = new ArrayList<WrapImageTextView.Box>();
	private final ArrayList<Area> mAreas = new ArrayList<WrapImageTextView.Area>();
	...
	protected void onDraw(Canvas canvas) {
		...
		ArrayList<HtmlObject> lineObjects = new ArrayList<WrapImageTextView.HtmlObject>();
	...
	private final ArrayList<Box> boxes = new ArrayList<WrapImageTextView.Box>();
	...
	int charCounter;
	float objPixelwidth;
	HashMap<Integer, HtmlObject> sorterMap = new HashMap<Integer, WrapImageTextView.HtmlObject>();


	private float parseSpans(ArrayList<HtmlObject> objects, Object[] spans, int lineStart, int lineEnd,
			float baseXOffset) {
		sorterMap.clear();
	...
	private final ArrayList<HtmlLink> mLinks = new ArrayList<WrapImageTextView.HtmlLink>();
	...
	private ArrayList<BitmapSpec> bitmaps = new ArrayList<WrapImageTextView.BitmapSpec>();
